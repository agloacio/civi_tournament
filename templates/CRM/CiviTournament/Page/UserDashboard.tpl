...
If you are new to this system, the first step is to double-check <a href="{crmURL p="{$component_path}/person" q="cid={$user->id}"}" target="_blank">your contact information</a>. You probably only need to do this once, ever.</li>
    ...
    <li><h4>Contacts (Players, coaches, etc.)</h4>The next step is to <em>enter</em> contacts for your group(s). You probably only need to do this once per contact, ever.
        ...
        You have access to contacts in these groups:        
            <ol>
                {foreach from=$user->registrationGroups item=group}
                    <li>{$group->name}
                        <br/><a href="{crmURL p="{$component_path}/group_member"     q="gid={$group->id}"}" target="_blank">List</a>
                        <br/><a href="{crmURL p="{$component_path}/group_member/add" q="gid={$group->id}"}" target="_blank">Add New</a>
                    </li>
                {/foreach}
            </ol>
    </li>

    <li><h4>Register Contacts for Tournament</h4>Once you have <em>entered</em> all the contacts for your group(s), you can <em>register</em> them to attend a tournament. (Be sure to indicate which competitions they will enter. That's important for the team registration step.)
        <p><a target="_blank" href="">Use this link to <em>register</em> a contact for the tournament</a>.</p>
        <p><a target="_blank" href="">Use this link to list/edit contacts already <em>registered</em> for the tournament.</a>.</p>
        <em>Note: </em>Tournament registration will close at : {$tournament->registrationEndDateTime->format('Y-m-d H:i:s')}.
    </li>
