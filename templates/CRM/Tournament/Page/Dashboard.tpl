{ts}
<h3>Tournament Dashboard</h3>

<ol>
    <li><h4>Your Contact Data</h4>Welcome, <a href="{$user->_contactUrl}" target="_blank">{$user->_name}</a>. The links on this page (and the Tournaments navigation menu above) will guide you through the steps of tournament registration.
If you are new to this system, the first step is to double-check your contact information. You probably only need to do this once, ever.</li>
    <li><h4>Billing Organizations (e.g., School Districts)</h4>The next step is to double-check your organization's contact information. You probably only need to do this once, ever.<p>You are a contact for these organizations:</p>
    <ol><li></li></ol>
    </li>
</ol>
{/ts}